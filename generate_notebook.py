...
cells.append(nbf.v4.new_markdown_cell(
    ...
    "This notebook explores and analyzes the Abstraction and Reasoning Corpus (ARC-AGI-2) dataset through the lens of **connected components**. "
    "We investigate how spatial adjacency rules (4-adjacent vs. 8-adjacent) and color grouping criteria (same-color vs. non-background) affect "
    "the structural properties of objects/motifs inside ARC matrices.\n\n"
    "By identifying these components programmatically, we quantify the underlying composition of puzzles, extract descriptive characteristics "
    "(such as size, shape, location, and border-touching status), and perform rigorous statistical hypothesis tests."
))

# --- CELL 2: Markdown (Preceding Code Cell 1) ---
cells.append(nbf.v4.new_markdown_cell(
    "## Step 1: Environment Setup & Library Imports\n\n"
    "In the following cell, we handle Google Colab compatibility (mounting Google Drive), define standardized output directories, "
    "and import all necessary scientific libraries. These include NumPy, Pandas, Matplotlib, Seaborn, and SciPy."
))
...
# --- CELL 4: Markdown (Preceding Code Cell 2) ---
cells.append(nbf.v4.new_markdown_cell(
    "## Step 2: Component Definition & Configuration\n\n"
    "Here we document what constitutes a **minimum component** and define configuration settings. "
    "A minimum component is defined as a set of connected pixels of non-background color with a size greater than or equal to `MIN_COMPONENT_SIZE`.\n\n"
    "**Documentation:**\n"
...
cells.append(nbf.v4.new_code_cell(
    ...
    "CONFIG = {\n"
    "    'MIN_COMPONENT_SIZE': 1,        # Minimum pixels to be a valid component\n"
...
# --- CELL 6: Markdown (Preceding Code Cell 3) ---
cells.append(nbf.v4.new_markdown_cell(
    "## Step 3: ARC Dataset Loading and Extraction\n\n"
    "We search Google Drive for the ARC dataset or download the official Kaggle ARC-AGI-2 repository from GitHub as a fallback "
    "to ensure no mock data is used. We then extract all challenge task files and load them into memory."
))
...
# --- CELL 8: Markdown (Preceding Code Cell 4) ---
cells.append(nbf.v4.new_markdown_cell(
    "## Step 4: Extract All Unique Grid Matrices\n\n"
    "In this cell, we extract every unique grid (both input and output) from the loaded ARC tasks. "
    "We store them with metadata regarding whether they belong to training or evaluation subsets, and whether they are input or output grids."
))
...
# --- CELL 10: Markdown (Preceding Code Cell 5) ---
cells.append(nbf.v4.new_markdown_cell(
    "## Step 5: Connected Components Labeling & Feature Extraction Logic\n\n"
    "We now implement the core algorithm to segment connected components. "
    "We separate between orthogonal (4-connected) and orthogonal+diagonal (8-connected) adjacencies, "
    "and between Same Color grouping and Non-Background grouping. "
    "For each labeled component, we extract sizes, shapes, bounding boxes, solidity, and centroids."
))
...
# --- CELL 12: Markdown (Preceding Code Cell 6) ---
cells.append(nbf.v4.new_markdown_cell(
...
# --- CELL 17: Markdown (Preceding Code Cell 7) ---
cells.append(nbf.v4.new_markdown_cell(
...
cells.append(nbf.v4.new_markdown_cell(
    "### Interpretation\n\n"
    "We observe that for both Same-Color and Non-Background grouping, the 8-adjacent configuration leads to significantly fewer "
    "components (smaller counts) and larger component sizes compared to 4-adjacent configuration. "
    "The extremely small p-values (< 0.05) allow us to confidently reject the null hypothesis $H_0$ in favor of $H_1$.\n\n"
    "**Insight**: Diagonal connectivity is an active element in ARC design. For example, a single diagonal line of pixels is segmented "
    "into size-1 components under 4-adjacency, but grouped into a single linear object under 8-adjacency. This shows how "
    "8-adjacency captures human-like cognitive spatial structures (like diagonal lines and shapes) much more naturally."
))
...
cells.append(nbf.v4.new_markdown_cell(
    "### Methodology\n\n"
    "We analyze components extracted under the 8-adjacent same-color rule and the 8-adjacent non-background rule. "
    "We measure and compare the following individual component features:\n"
...
# --- CELL 23: Markdown (Preceding Code Cell 8) ---
cells.append(nbf.v4.new_markdown_cell(
    "### Results and Visualizations\n\n"
...
cells.append(nbf.v4.new_markdown_cell(
    "### Interpretation\n\n"
    "The Mann-Whitney U test results demonstrate a highly significant difference in sizes, solidity, and aspect ratios. "
    "With extremely low p-values (< 0.05), we reject the null hypothesis $H_0$ in favor of $H_1$.\n\n"
    "**Insights**:\n"
    "1. **Sizes**: Same-color components are significantly smaller on average than non-background components. This is because multi-colored composite objects "
    "(such as a box with a different-colored center, or a striped structure) are broken down into individual monochromatic layers/shapes "
    "when using the Same-Color rule.\n"
    "2. **Solidity**: Same-color components exhibit substantially higher solidity/compactness. Non-background components often contain background holes (0s) or "
    "are complex multi-colored meshes, resulting in lower solidity (less dense packing relative to their bounding box).\n"
    "3. **Aspect Ratio**: The aspect ratio distributions also differ, highlighting that multi-colored structures have more complex aspect ratios, "
    "whereas same-color parts tend to represent more primitive geometric forms."
))

# --- CELL 26: Markdown ---
cells.append(nbf.v4.new_markdown_cell(
    "# Advanced Statistics: Component Locations, Border-Touching and Colors in Puzzles\n\n"
    "This section delivers further empirical analysis on where components are placed inside puzzles, what colors they are, and "
    "how many of them touch the grid boundary."
))

# --- CELL 27: Markdown (Preceding Code Cell 9) ---
cells.append(nbf.v4.new_markdown_cell(
    "### Component Location and Centroid Density Analysis\n\n"
    "We plot a 2D density map of normalized centroids of the components. This reveals whether components tend to cluster in the center "
    "of the grid or are distributed uniformly. We also examine the distribution of colors and border-touching frequencies."
))
...
# --- CELL 29: Markdown (Preceding Code Cell 10) ---
cells.append(nbf.v4.new_markdown_cell(
    "### Step 7: Exporting Analysis Summary & Results\n\n"
    "Finally, we serialize key metrics and distributions of the analysis to Google Drive as requested by the research standards. "
    "We save a structured summary JSON file containing overall characteristics of components in ARC."
))

# --- CELL 30: Code Cell 10 ---
cells.append(nbf.v4.new_code_cell(
    "summary_metrics = {\n"
...
# Save notebook
notebook_path = '1.Connected_Components_Analysis.ipynb'
